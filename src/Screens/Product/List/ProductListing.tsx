...
const ProductListing = ({ data, onDelete }: any) => {
    ...
    return (
        <div className="flex justify-center items-center h-screen bg-gray-50">
            ...
            {location.pathname !== '/layout/product-list'} ? <Outlet/> : <div className="w-full max-w-4xl p-6 bg-white border border-gray-200 rounded-lg shadow-md">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-semibold">Products</h2>
                    <Link to="/layout/add-product">
                            ...
                            data?.data?.map((product: any) => (
                                <tr key={product._id} style={{ textAlign: "center" }} className='text-slate-900'>
                                    ...
                                    <td className="py-2 px-4 border-b">
                                        <Link
                                            to={`/layout/edit-product/${product._id}?photo=${product.photo}&productName=${product.productName}&type=${product.type}&quantity=${product.quantity}&mrp=${product.mrp}&rate=${product.rate}`}
                                        >
